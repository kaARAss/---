document.addEventListener('DOMContentLoaded', function() {
    ...
    function createEventItem(monthData, index) {
        ...
        button.addEventListener('click', function(e) {
            ...
            eventsSection.appendChild(eventTickets);
            openEventTickets(monthYear);
        });
    ...
    function createEventTickets(monthData, index) {
        ...
        const prevMonthText = monthNames[prevMonthKey];

        prevText.textContent = prevMonthText;
        prev.innerHTML = `${prevIcon}${prevText.outerHTML}`;
        ...
        const nextMonthKey = String(nextMonthNum).padStart(2, "0");
        const nextMonthText = monthNames[nextMonthKey];

        nextText.textContent = nextMonthText;
        next.innerHTML = `${nextText.outerHTML}${nextIcon}`;
        ...
        closeBtn.addEventListener('click', function(e) {
            e.preventDefault();
            eventTickets.remove();
            // bgOverflow.classList.remove('active');
            document.body.classList.remove('no-scroll');
        });

        eventTitle.appendChild(navigation);
        ...
        eventTickets.appendChild(eventsList);
    ...
    function openOrCreateEventTickets(targetIndex) {
        ...
        if (!modal) {
            ...
            eventsSection.appendChild(modal);
        }
        openEventTickets(monthYear);
    }
    ...
    function openEventTickets(monthYear) {
        ...
        if (!eventTickets) {
            console.warn('Не найден блок event_tickets для:', monthYear);
            return;
        }

        const [monthNum, year] = monthYear.split('_');
        modalName = monthYear;
        ...
        loadEventsList(eventTickets, monthNum, year);
    }
